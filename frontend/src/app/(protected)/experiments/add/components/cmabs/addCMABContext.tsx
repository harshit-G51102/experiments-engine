import {
  ...
  import { CMABExperimentState, NewCMABArm, StepComponentProps, ContextType, NewContext } from "../../../types";
...
import { Radio, RadioGroup, RadioField } from "@/components/catalyst/radio";
  ...
  export default function AddCMABContext({ onValidate }: StepComponentProps) {
    ...
    const [errors, setErrors] = useState(() => {
        if (methodType === "cmab" && experimentState.contexts && Array.isArray(experimentState.contexts)) {
    ...
    const contexts = methodType === "cmab" && experimentState.contexts
                ...
                setExperimentState((prevState) => {
                  if (prevState.methodType === "cmab") {
                    ...
                    contexts: [
                        ...
                        { ...defaultContext, context_id: Date.now() } as NewContext,
                    ],
                    }
                  } else {
                    console.error("Method type is not CMAB");
              ...
              >
                ...
                <Field className="flex flex-col mb-4">
                    ...
                    <RadioGroup
                      ...
                      typeSafeSetExperimentState(newContexts as NewContext[]);
                    }}
                    >
                      <div className="space-y-2">
                        <RadioField className="flex items-start space-x-2 rounded-md border border-gray-800 p-3 hover:bg-gray-800/50 transition-colors">
                          <Radio id={`context-${index}-binary`} value="binary" />
                        ...
                        <RadioField className="flex items-start space-x-2 rounded-md border border-gray-800 p-3 hover:bg-gray-800/50 transition-colors">
                          <Radio id={`context-${index}-real-valued`} value="real-valued" />
                            <div className="flex flex-col">
                    ...
                    {errors[index]?.value_type ? (
                        <p className="text-red-500 text-xs mt-1">{errors[index].value_type}</p>
                    ) : (
                        <p className="text-red-500 text-xs mt-1">&nbsp;</p>
                    )}
